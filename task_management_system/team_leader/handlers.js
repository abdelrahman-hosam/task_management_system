...
const canAssign = async(teamId, taskId, teamLeaderId, [members])=>{
    ...
    const teamMembersQuery = await db.nosqlDB.collection('team').findOne({id: teamId} , {projection: {teamMembers: 1, assignedTasks:1, teamLeaderId: 1 , _id: 0}})
    ...
    if(teamLeaderId !== teamMembersQuery.teamLeaderId) return false
...
const showAssignedTasks = async(req, res)=> {
    try{
        ...
        if(!teamId) return res.status(404).json({'message':'team leader does not have any assigned team to him'})
        const team = await db.nosqlDB.collection('team').findOne({id: teamId}, {projection: {assignedTasks: 1, teamLeaderId: 1 , _id: 0}})
        if(!team) return res.status(404).json({'message':`team with the team id ${teamId} does not have tasks assigned to it`})
        if(team.teamLeaderId !== id) return res.status(401).json({'message':'you are not authorized to do this action'})
        return res.status(200).json({'message':`team with the team id ${teamId} has the tasks ${team.assignedTasks} assigned to them`})
    }catch(err){
