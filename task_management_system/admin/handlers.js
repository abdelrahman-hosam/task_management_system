...
const createTeam = async(req , res)=> {
    try{
        ...
        const [{insertId}] = await db.mysqlDB.promise().query('INSERT INTO team(head_manager_id, team_leader_id) VALUES (?,?)' , [head_manager, team_leader])
        const nosql = await db.nosqlDB()
        await nosql.collection('team').insertOne({id: String(insertId), teamLeaderId: team_leader, teamMembers:[]})
...
const createTeamMember = async(req, res) => {
    try{
        ...
        const [{insertId}] = await mysql.promise().query('INSERT INTO team_member(employee_name, birthdate, email, team_id) VALUES(?,?,?,?)', [name, birthdate, email, team_id])
        const nosql = await db.nosqlDB()
        await nosql.collection('team').updateOne({id: team_id}, {$push: {teamMembers: String(insertId)}})
        return res.status(201).json({'message':`A team member is created with id ${insertId}`})
    }catch(err){
        return res.status(500).json({'message':`something went wrong ${err}`})
    }
}
...
const removeTeamLeader = async(req,res) => {
    try{
        ...
        const areValid = validTypes('string', [id])
        if(!areValid) return res.status(400).json({'message':'insert the fields in the required form'})
        const [deletedSql] = await db.mysqlDB.promise().query('DELETE FROM team_leader WHERE id = ?' , [id])
        const deletedNoSql = await db.nosqlDB.collection('team').updateOne({teamLeaderId: id} , {$unset: {teamLeaderId: ""}})
        if(deletedSql.affectedRows < 1) return res.status(404).json({'message':'team does not exist'})
        if(deletedNoSql.modifiedCount < 1) return res.status(404).json({'message':'team leader assigned teams were not found'})
        return res.status(200).json({'message':`Team with id ${id} is deleted`})
    }catch(err){
        return res.status(500).json({'message':`something went wrong ${err}`})
    }
...
const showFromDB = async(req , res) => {
    try{
        ...
        return res.status(200).json({'message':`data requested succesfully`, data: data})
    }catch(err){
        return res.status(500).json({'message':`something went wrong ${err}`})
    }
}
