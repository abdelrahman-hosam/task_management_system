...
const assignTask = async(req , res) => {
    try{
        ...
        if(!team)return res.status(404).json({'message':`The error could be one of the following: the manager is not found or the team is not found or the manager does not have a premission to assign tasks to this team`})
        //if no team leader assigned to the team
        if(!team.team_leader_id)return res.status(400).json({'message':'tasks can not be assigned to this team as it has no team leader'})
        //create task id 
        let id
